...
#ifndef LIBBSP_ARM_TMS570_DMA_H
#define LIBBSP_ARM_TMS570_DMA_H
...
/**
 * @defgroup tms570_dma DMA Support
 *
 * @ingroup tms570
 *
 * @brief Direct memory access (DMA) support.
 ...
 */
rtems_status_code tms570_dma_channel_obtain( unsigned channel );

/**
 * @brief Releases the DMA channel @a channel.
 *
 * You must have obtained this channel with tms570_dma_channel_obtain()
 * previously.
 *
 * If the channel number @a channel is out of range nothing will happen.
 */
void tms570_dma_channel_release( unsigned channel );
...
rtems_status_code tms570_dma_copy_wait( unsigned channel );
