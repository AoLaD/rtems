...
static rtems_id tms570_dma_sema_table[ GPDMA_CH_NUMBER ];
static bool     tms570_dma_status_table[ GPDMA_CH_NUMBER ];
...
static void tms570_dma_copy_handler( void *arg )
{
  ...
  if ( ( tc & GPDMA_STATUS_CH_0 ) != 0 ) {
    rtems_semaphore_release( tms570_dma_sema_table[ 0 ] );
  }

  /* else implies that the channel is not the 0. Also,
     there is nothing to do. */

  tms570_dma_status_table[ 0 ] = ( err & GPDMA_STATUS_CH_0 ) == 0;

  if ( ( tc & GPDMA_STATUS_CH_1 ) != 0 ) {
    rtems_semaphore_release( tms570_dma_sema_table[ 1 ] );
  }

  /* else implies that the channel is not the 1. Also,
     there is nothing to do. */

  tms570_dma_status_table[ 1 ] = ( err & GPDMA_STATUS_CH_1 ) == 0;
}
...
rtems_status_code tms570_dma_copy_initialize( void )
{
  ...
  status_code = rtems_interrupt_handler_install( TMS570_IRQ_DMA,
    "DMA copy",
    RTEMS_INTERRUPT_UNIQUE,
    tms570_dma_copy_handler,
    NULL );
  ...
  tms570_dma_sema_table[ 0 ] = id0;
  tms570_dma_sema_table[ 1 ] = id1;
...
rtems_status_code tms570_dma_copy_release( void )
{
  ...
  status_code = rtems_interrupt_handler_remove( TMS570_IRQ_DMA,
    tms570_dma_copy_handler,
    NULL );
  ...
  status_code = rtems_semaphore_delete( tms570_dma_sema_table[ 0 ] );
  ...
  status_code = rtems_semaphore_delete( tms570_dma_sema_table[ 1 ] );
...
{
  rtems_status_code             status_code = RTEMS_SUCCESSFUL;
  volatile tms570_dma_channel *e = GPDMA_CH_BASE_ADDR( channel );
...
rtems_status_code tms570_dma_copy_wait( const unsigned channel )
{
  ...
  status_code = rtems_semaphore_obtain( tms570_dma_sema_table[ channel ],
    RTEMS_WAIT,
    RTEMS_NO_TIMEOUT );
  ...
  status_code = tms570_dma_status_table[ channel ]
