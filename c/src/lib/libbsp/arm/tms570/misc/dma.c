 ...
 */
static bool tms570_dma_channel_occupation[ GPDMA_CH_NUMBER ];

void tms570_dma_initialize( void )
{
  /* Enable module power */
  tms570_module_enable( TMS570_MODULE_GPDMA, TMS570_MODULE_PCLK_DEFAULT );
...
rtems_status_code tms570_dma_channel_obtain( const unsigned channel )
{
  ...
  if ( channel < GPDMA_CH_NUMBER ) {
    ...
    occupation = tms570_dma_channel_occupation[ channel ];
    tms570_dma_channel_occupation[ channel ] = true;
...
{
  if ( channel < GPDMA_CH_NUMBER ) {
    tms570_dma_channel_occupation[ channel ] = false;
  }
...
{
  if ( channel < GPDMA_CH_NUMBER ) {
    volatile tms570_dma_channel *ch = GPDMA_CH_BASE_ADDR( channel );
    uint32_t                      cfg = ch->cfg;
